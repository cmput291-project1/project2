...
public class RangeSearch{
	...
	public void compare(){
		...
		this.lowerLimit = Interval.LOWER_LIMIT;
		this.upperLimit = Interval.UPPER_LIMIT;
	...
	public void findInterval() throws DatabaseException{
		this.lowerLimit = Interval.LOWER_LIMIT;

		Cursor cursor = dataBase.openCursor(null, null);
		if(cursor == null){
			throw new RuntimeException("cursor opened in RangeSearch.findInterval() is null");
		}
		DatabaseEntry key = new DatabaseEntry();
		DatabaseEntry data = new DatabaseEntry();
		...
		if(oprStatus == OperationStatus.SUCCESS){
			System.out.println("lower limit found");
			System.out.println("lower limit key: " + new String(key.getData()));
			int count = 1;
			while(oprStatus == OperationStatus.SUCCESS && count != 101){
				count++;
				oprStatus = cursor.getNext(key, data, LockMode.DEFAULT);
			}
			System.out.println("upper limit found");
			System.out.println("upper limit key: " + new String(key.getData()));
		}
		else{
			System.out.println("lower limit is not found: " + new String(key.getData()));
		}
